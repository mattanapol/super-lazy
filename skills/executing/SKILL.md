...
1. **Workspace.** As in §1: use `ledger:using-git-worktrees`, and never start implementation on main/master without your human partner's explicit consent. There is no `.unlazy/<scope>/` and no scoped copy — the committed `docs/plans/…` file is the plan, and it is the one you read and the one whose task checkboxes you tick. There is no brief either, so `task-brief` has nothing to extract for. `review-package` still works at step 5 and derives its own output directory under `.ledger/sdd/<plan-basename>/` through `sdd-workspace`; that directory is git-ignored scratch you delete once the review is clean.
...
Dispatch [task-reviewer-prompt.md](./task-reviewer-prompt.md) with the brief path, the report path, the package path, and the global constraints binding this leaf — copied verbatim from `PLAN.md`'s Contract — the section `ledger:planning` calls Contract and the reviewer template calls global constraints; they are the same block — with its exact values, exact formats, and stated relationships between components. The template already carries the process rules; the constraints block is for what *this* project's spec demands. Never dispatch a task reviewer without a diff file.
...
Only layers 2 and 3 are independent of the leaf. Layer 1 is the leaf grading its own work. Layer 4 is a scan-only backstop; it never runs a check and never judges whether an oracle measures its English outcome. Install it with `/gates-enforce`, never by running the installer yourself: that command explains what changes, takes explicit consent before writing anything, handles the pin/bind branch a multi-scope project needs, and reads back `.claude/settings.local.json` to confirm. A bare install in a multi-scope project produces a hook that silently never blocks, which looks exactly like a passing session. It is optional because it adds nothing a disciplined loop is not already doing — and worth having because the compacted driver is precisely the one that would otherwise stop early. On an abandonment it allows the stop and emits a bounded `HANDOFF REQUIRED` message naming the ledger or wave.
